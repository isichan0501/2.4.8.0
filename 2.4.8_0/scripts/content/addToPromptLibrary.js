...
function updateSubmitButton() {
  const submitButton = document.getElementById('submit-prompt-button');
  ...
  if (selectedCategories.length > 0 && promptText?.value.trim().length > 0 && promptTitle?.value.trim().length > 0 && nickname?.value.trim().length > 0 && url?.value.trim().length > 0) {
    submitButton.disabled = false;
    submitButton.style.color = 'white';
    submitButton.title = '';
  } else {
    submitButton.disabled = true;
    submitButton.style.color = '#565869';
    submitButton.title = 'Please fill out all fields';
  }
}

function openSubmitPromptModal(text, promptId = null, title = '', categories = [], language = '', hideFullPrompt = false) {
  selectedCategories = categories;
  ...
  languageSelectorLabel.textContent = 'What language is this prompt in?';
  ...
  chrome.storage.local.get(['settings'], (result) => {
    ...
    languageSelectorWrapper.innerHTML = dropdown('Prompt-Language', promptLanguageList, selectedPromptLanguage);
  ...
  urlInput.placeholder = 'Your Twitter, Github, personal website, etc.';
  ...
  submitButton.style = 'width:100%;height:40px;background-color:#0b0d0e;border:1px solid #565869;border-radius:4px;color:#565869;margin-top:16px;margin-left:8px;';
  ...
  submitButton.addEventListener('click', () => {
    ...
    chrome.storage.local.get(['settings'], (res) => {
      chrome.storage.sync.get(['openai_id'], (result) => {
        ...
        submitPrompt(result.openai_id, textToSubmit.value, promptTitleInput.value, selectedCategories, res.settings.selectedPromptLanguage.code, nicknameInput.value, urlInput.value, curHideFullPromptSwitch?.checked || false, promptId).then((data) => {
