...
function generateChat(message, conversationId, messageId, parentMessageId) {
...
function submitPrompt(openAiId, prompt, promptTitle, categories, promptLangage, nickname, url, hideFullPrompt = false, promptId = null) {
...
function getPrompts(pageNumber, searchTerm, sortBy = 'recent', language = 'all', category = 'all') {
  ...
  return chrome.storage.sync.get(['user_id']).then((result) => {
    const userId = result.user_id;
    let url = `${API_URL}/gptx/?order_by=${sortBy}`;
    if (sortBy === 'mine') url = `${API_URL}/gptx/?order_by=${sortBy}&id=${userId}`;
...
function updateEmailNewsletter(emailNewsletter) {
  ...
  chrome.storage.sync.get(['openai_id'], (result) => {
    ...
    return fetch(url, {
      ...
      body: JSON.stringify({ p: openaiId.split('-')[1], email_newsletter: emailNewsletter }),
    });
  });
}
