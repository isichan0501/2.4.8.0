...
/* global updateNewChatButtonNotSynced, getAllConversations, getConversation, loadConversationList, initializeCopyAndCounter, initializeAddToPromptLibrary, initializeTimestamp, addConversationsEventListeners, isGenerating, prependConversation, generateTitleForConversation, canSubmitPrompt, formatDate, userChatIsActuallySaved:true, addAsyncInputEvents */
    ...
    if (existingConversation) {
      ...
      chrome.storage.local.set(
        ...
        () => {
          ...
          if (generateTitle && existingConversation.title === 'New chat' && mapping.length < 5 && mapping.filter((m) => m.message?.author.role === 'assistant').length === 1) { // only one assistant message
          ...
          } else if (settings.conversationSort) { // === updated
            ...
            const conversationTimestamp = conversationElement.querySelector('#timestamp');
            conversationTimestamp.innerHTML = conversationCreateTime;
    ...
    chrome.storage.local.get(['conversations'], (result) => {
      ...
      chrome.storage.local.set({
        ...
        for (let i = 0; i < localConvIds.length; i += 1) {
          ...
          if (typeof localConversations[localConvIds[i]].folderId === 'undefined') {
            localConversations[localConvIds[i]].folderId = null;
          }
