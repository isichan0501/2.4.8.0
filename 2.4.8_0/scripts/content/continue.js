...
function updateContinueButton() {
  ...
  const continueButtonClone = continueButton.cloneNode(true);
  continueButton.replaceWith(continueButtonClone);

  chrome.storage.local.get('customPrompts', ({ customPrompts }) => {
    continueButtonClone.textContent = Array.isArray(customPrompts) ? customPrompts.find((p) => p.isDefault)?.title || 'Continue' : 'Continue';
  });
  continueButtonClone.addEventListener('click', () => {
    chrome.storage.local.get('customPrompts', ({ customPrompts }) => {
      const main = document.querySelector('main');
      if (!main) return;
      const inputForm = main.querySelector('form');
...
function initializeContinue() {
  ...
  chrome.storage.onChanged.addListener((e) => {
    if (e.customPrompts) {
      ...
      const newDropdown = promptDropdown();
      const oldDropdown = document.getElementById('continue-conversation-dropdown-list');
