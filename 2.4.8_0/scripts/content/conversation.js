    ...
    chrome.storage.local.get(['conversations', 'settings'], (res) => {
      ...
      for (let i = 0; i < sortedNodes.length; i += 1) {
        ...
        if (message.role === 'assistant' || message.author?.role === 'assistant') {
          messageDiv += rowAssistant(fullConversation, sortedNodes[i], threadIndex, threadCount, searchValue);
        }
      }
    ...
    button.addEventListener('click', () => {
      chrome.storage.local.get(['conversations', 'settings'], (result) => {
        ...
        saveButton.addEventListener('click', () => {
          ...
          isGenerating = true;
          submitChat(newMessage, conversation, newMessageId, parentId, result.settings);
          // }
        });
