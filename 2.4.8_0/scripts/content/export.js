...
function addExportAsButton(title) {
  ...
  exportAsButton.addEventListener('click', () => {
    ...
    // check if conversation id is valid uuid v4
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(conversationId)) {
      chrome.storage.local.get(['conversations', 'conversationsAreSynced', 'settings']).then((res) => {
        ...
        if (conversationsAreSynced) {
          const lastConversation = showArchivedConversations
            ? Object.values(storageConversations).filter((c) => !c.skipped).sort((a, b) => new Date(b.create_time).getTime() - new Date(a.create_time).getTime())[0]
            : Object.values(storageConversations).filter((c) => !c.archived && !c.skipped).sort((a, b) => new Date(b.create_time).getTime() - new Date(a.create_time).getTime())[0];
          getSingelConversation(lastConversation.id, title);
        } else {
  ...
  chrome.storage.local.get(['selectedConversations'], (result) => {
    ...
    if (selectedConversations?.length > 0) {
      ...
      chrome.storage.local.get(['conversations', 'conversationsAreSynced', 'settings']).then((res) => {
        ...
        if (conversationsAreSynced) {
          const visibleConversations = showArchivedConversations
            ? Object.values(storageConversations).filter((conversation) => !conversation.skipped)
            : Object.values(storageConversations).filter((conversation) => !conversation.archived && !conversation.skipped);
          exportAllModalProgressBarLabel.textContent = `0 / ${visibleConversations.length}`;
          exportAllModalExportButton.disabled = visibleConversations.length === 0;
          exportAllModalExportButton.style.opacity = visibleConversations.length === 0 ? '0.5' : '1';
        } else {
          getConversations(0, 1).then((conversations) => {
