/* global markdownit, hljs, resetSelection, newChatPage, initializeRegenerateResponseButton, notSelectedClassList, textAreaElementInputEventListener, addExamplePromptEventListener, textAreaElementKeydownEventListener */
...
function addNavToggleButton() {
  chrome.storage.local.get(['settings'], (result) => {
    ...
    const mainContent = document.querySelector('.md\\:pl-\\[260px\\]');
    if (!sidebar) return;
    if (!mainContent) return;
    // add transition to nav and main
    sidebar.style = `${sidebar.style.cssText};transition:margin-left 0.3s ease-in-out;`;
      ...
      chrome.storage.local.get(['settings'], (res) => {
        ...
        chrome.storage.local.set({
          ...
          if (newNavOpen) {
            ...
            const main = document.querySelector('.md\\:pl-0');
            ...
            const nav = document.querySelector('.md\\:w-\\[260px\\]');
            const main = document.querySelector('.md\\:pl-\\[260px\\]');
...
function showNewChatPage() {
  ...
  chrome.storage.local.get(['conversationsAreSynced', 'account'], (result) => {
    ...
    const { pathname, href } = new URL(window.location.toString());
    if (href !== 'https://chat.openai.com/chat') {
      window.history.pushState({}, '', 'https://chat.openai.com/chat');
    }
...
function removeUnusedButtons() {
  ...
  const updatesButton = allNavButtons.find((button) => button.textContent.toLocaleLowerCase() === 'updates & faq');
...
function toast(text, type = 'info') {
  ...
  element.style = 'position:fixed;right:24px;top:24px;border:1px solid gold;border-radius:4px;background-color:#565869;padding:4px 8px;color:gold;z-index:100001;';
  if (type === 'error') {
    element.style.borderColor = '#ff4a4a';
    element.style.color = '#ff4a4a';
    // element.style.backgroundColor = '#864e6140';
  }
