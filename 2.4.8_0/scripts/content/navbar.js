/* global modelSwitcher, dropdown, addDropdownEventListener, languageList, writingStyleList, toneList, addModelSwitcherEventListener */
function addNavbar() {
  const existingNavbar = document.querySelector('#gptx-navbar');
  ...
  chrome.storage.local.get(['settings', 'models', 'account', 'unofficialModels', 'customModels'], (result) => {
    const {
      models, settings, account, unofficialModels, customModels,
    } = result;
    const {
    ...
    toneSelectorWrapper.innerHTML = dropdown('Tone', toneList, selectedTone);
    ...
    writingStyleSelectorWrapper.innerHTML = dropdown('Writing-Style', writingStyleList, selectedWritingStyle);
    ...
    languageSelectorWrapper.innerHTML = dropdown('Language', languageList, selectedLanguage);
