...
function createPromptHistoryModal() {
  chrome.storage.local.get(['userInputValueHistory', 'settings'], (result) => {
    ...
    createModal('My Prompt History', 'All your personal and favorite prompts are saved here.', bodyContent, actionsBarContent, true);
  ...
  if (userInputValueHistory && userInputValueHistory.length > 0) {
    userInputValueHistory.reverse().forEach((userInputValue, index) => {
      ...
      historyItemSubmitButton.addEventListener('click', () => {
        ...
        openSubmitPromptModal(userInputValue.inputValue);
      });
