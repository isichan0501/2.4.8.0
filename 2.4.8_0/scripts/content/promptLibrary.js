/* global highlight, highlightBracket, addUserPromptToHistory, addButtonToNavFooter,createModal, debounce, deletePrompt, incrementUseCount, getPrompts, report, vote, toast, openSubmitPromptModal, dropdown, addDropdownEventListener, languageList, categoryList, sortByList */
...
function promptLibraryListComponent(libraryData, loading = false) {
  ...
  libraryData.results.forEach((libraryPrompt) => {
    ...
    libraryItemEditButton.addEventListener('click', () => {
      openSubmitPromptModal(libraryPrompt.text, libraryPrompt.id, libraryPrompt.title, libraryPrompt.categories, libraryPrompt.language, libraryPrompt.hide_full_prompt);
    });
    // delete button
...
function promptLibraryModalContent(libraryData) {
  ...
  chrome.storage.local.get(['settings'], (result) => {
    ...
    sortBySelectorWrapper.innerHTML = dropdown('Library-SortBy', sortByList, selectedLibrarySortBy, true);
  ...
  chrome.storage.local.get(['settings'], (result) => {
    ...
    categorySelectorWrapper.innerHTML = dropdown('Library-Category', categoryList, selectedLibraryCategory, true);
  ...
  chrome.storage.local.get(['settings'], (result) => {
    ...
    languageSelectorWrapper.innerHTML = dropdown('Library-Language', libraryLanguageList, selectedLibraryLanguage, true);
