/* global createModal, updateEmailNewsletter, createReleaseNoteModal, languageList, writingStyleList, toneList, toast, loadConversationList, modelSwitcher, addModelSwitcherEventListener */
...
function generalTabContent() {
  ...
  // safe mode
  const safeModeSwitch = createSwitch('Safe Mode', 'Enable/disable ChatGPT Safe mode.(requires Chrome)', 'safeMode', true, safeModeTrigger, 'Beta');
  ...
  updatesLink.textContent = 'Updates & FAQ ➜';
  ...
  faqLink.href = 'https://ezi.notion.site/FAQ-9d43a8a1c31745c893a4080029d2eb24';
...
function autoSyncTabContent() {
  ...
  chrome.storage.local.get(['settings'], (result) => {
    ...
    const conversationSortSwitch = createSwitch('Conversation Sort', 'OFF: Sort by Created time, ON: Sort by last updated time', 'conversationSort', false, reloadConversationList, 'Requires Auto-Sync', !autoSync);
    content.appendChild(conversationSortSwitch);
    ...
    // show archived conversations mode
    const showArchivedConversationsSwitch = createSwitch('Show Archived Conversations', 'Show/Hide the deleted ChatGPT conversations', 'showArchivedConversations', false, reloadConversationList, 'Requires Auto-Sync', !autoSync);

    content.appendChild(showArchivedConversationsSwitch);
  });
  return content;
}
...
function modelsTabContent() {
  ...
  modelSwitcherWrapper.style = 'position:relative;width:300px;z-index:1000; pointer-events: none;';
  const idPrefix = 'settings';
  modelSwitcherWrapper.id = `model-switcher-wrapper-${idPrefix}`;
  content.appendChild(modelSwitcherWrapper);
  ...
  newCustomModelSaveButton.addEventListener('click', (e) => {
    ...
    chrome.storage.local.get(['models', 'unofficialModels', 'customModels', 'settings'], (res) => {
      ...
      newCustomModels.push({ title: `${newCustomModelSlug.value.trim()} (custom)`, description: newCustomModelText.value.trim(), slug: newCustomModelSlug.value.trim() });
  ...
  chrome.storage.local.get(['settings', 'presetPrompts', 'selectedConversations', 'customPrompts'], (result) => {
    ...
    chrome.storage.local.set({
      settings: {
        ...
        conversationSort: result.settings?.conversationSort !== undefined ? result.settings.conversationSort : false,
        showArchivedConversations: result.settings?.showArchivedConversations !== undefined ? result.settings.showArchivedConversations : false,
